    ...
    , warning (newWarning)
{
    ...
    if (pressurehexProcessor != nullptr)
    {
        ...
        {
            pressurehexProcessor->setAuditionEnabled (! pressurehexProcessor->isAuditionEnabled());
        ...
        {
            pressurehexProcessor->setAuditionType (1 - pressurehexProcessor->getAuditionType());
            syncAuditionControls();
        };
...
void ParameterGridEditor::timerCallback()
{
    ...
    {
        const auto latestInputPeak = pressurehexProcessor->getInputPeakLevel();
        const auto latestPeak = pressurehexProcessor->getOutputPeakLevel();
...
{
#if defined(YUP_AUDIO_PLUGIN_ENABLE_STANDALONE)
    if (pressurehexProcessor == nullptr || auditionButton == nullptr || auditionTypeButton == nullptr)
        return;

    auditionButton->setButtonText (pressurehexProcessor->isAuditionEnabled() ? "Audition On" : "Audition Off");
    auditionTypeButton->setButtonText (pressurehexProcessor->getAuditionType() == 0 ? "Saw" : "Pulse");
    static_cast<ehl::ui::CommandButton*> (auditionButton.get())->setSelected (pressurehexProcessor->isAuditionEnabled());
    static_cast<ehl::ui::CommandButton*> (auditionTypeButton.get())->setSelected (pressurehexProcessor->getAuditionType() != 0);
#endif
}
